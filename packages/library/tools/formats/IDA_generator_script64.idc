...
and RequestTimeout <https://github.com/RequestTimeout408>.
...
static parseMember(iAddress, iParsedStructsId, iOutputFile)
{
    ...
    else if (aTypeId == 0x01)
    {
		...
		psAdr = Qword(iAddress + 16);
		
		///Basic
		if (Byte(Qword(psAdr + 8)) == 0){
			aTempOutput = form("'%s', ['[]', %s, %d]", aMemberName, parseStruct2(psAdr, iParsedStructsId, iOutputFile, 0), Dword(iAddress + 24));
		}
		
		///Non-basic
		else{
			aTempOutput = form("'%s', ['[]', this.%s, %d]", aMemberName, parseStruct2(psAdr, iParsedStructsId, iOutputFile, 0), Dword(iAddress + 24));
		}
...
static parseStructTab(iANSTructTabOffset, iNbOfVersions, iOutputFile)
{
    ...
    while (aLoopIndex >= 0 && aLoopIndex >= iNbOfVersions - historyDepth)
    {
		...
		aSubAddress = Qword(iANSTructTabOffset + 24 * aLoopIndex + 8);
