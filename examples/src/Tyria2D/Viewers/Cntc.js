...
class CntcViewer extends Viewer {
    ...
    w2ui.cntcLayout.content(
      ...
      $().w2grid({
        ...
        onClick: function (event) {
          ...
          const record = w2ui.contentList.records[event.recid];
          console.log(record);
  ...
  render() {
    ...
    if (this.currentRenderId !== fileId) {
      ...
      const mainChunk = packfile.chunks[0].data;
      const entryArray = [];
      ...
      for (let i = 0; i < entriesAmount; i++) {
        const begin = mainChunk.indexEntries[i].offset;
        const end = mainChunk.indexEntries[i + 1] ? mainChunk.indexEntries[i + 1].offset : mainChunk.content.length;
